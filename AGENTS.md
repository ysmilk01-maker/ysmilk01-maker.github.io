...
이 저장소가 사이트의 **원본**이다. 빌드 단계도, 프레임워크도, npm 도 없다.
HTML·CSS·JS 파일이 곧 사이트다.

**나가는 곳이 두 군데다. 2026-09-11 부터 바뀌었다.**

| 주소 | 나가는 방법 | 쓰임 |
| --- | --- | --- |
| **https://ninetosixlab.com** | 플러그인 zip 을 만들어 워드프레스에 올린다 | **실제 서비스 주소** |
| https://ysmilk01-maker.github.io | `main` 에 푸시하면 1~2분 뒤 자동 | `lab/` 의 실제 게임, 원본 보관 |

**`main` 에 푸시해도 `ninetosixlab.com` 은 바뀌지 않는다.** 그 사이트는 카페24
워드프레스에서 돌고, 자체 플러그인 `ntsl-site` 가 이 저장소의 정적 HTML 사본을
들고 있다. 고친 내용을 내보내려면 플러그인을 다시 빌드해서 올려야 한다.

```
git push                                 # 깃허브 페이지 + 원본 보관
python wordpress/build-plugin.py --bump  # 플러그인 zip 만들기
# → wp-admin > 플러그인 > 새로 추가 > 플러그인 업로드
# → 「현재 플러그인을 업로드한 것으로 교체」
# → 설정 > WP Super Cache > 내용 > 「캐시 삭제」   ← 빼먹으면 옛 화면이 계속 보인다
```

빌드 스크립트와 플러그인 원본은 저장소의 `wordpress/`에 있다.
분석 태그가 빠지지 않도록 외부의 예전 이식 패키지로 빌드하지 않는다.
자세한 절차는 `docs/ANALYTICS-20260913.md`를 참고한다.
`lab/` 은 17MB 라 플러그인에 담지 않는다 — 워드프레스 쪽 게임 링크는 깃허브
페이지를 가리킨다.
...
- [ ] **푸시 뒤 플러그인을 다시 빌드해 올렸다** — 안 하면 `ninetosixlab.com` 은 그대로다
