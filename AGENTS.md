...
혼자 만드는 1인 스튜디오의 사이트다. 토스 미니앱 네 개와 웹 스도쿠를 소개한다. 코드에 있는
...
| `tools/build_pages.py` | 상세 페이지 다섯 장의 내용과 틀. 글을 고치려면 여기 `APPS` 를 고치고 `python tools/build_pages.py` |
...
| `img/` | 앱 스크린샷(`*.png` 1170×2532), 스토어 샷(`shots/*.webp`), 첫 화면 조형물(`hero/*.webp`) |
...
- 검정 바탕, Pretendard, `word-break: keep-all`. 앱마다 색이 하나씩 있고(`--coin`
  `--krx` `--saju` `--ai` `--sudoku`) 그 색은 그 앱의 카드·페이지 안에서만 켠다
