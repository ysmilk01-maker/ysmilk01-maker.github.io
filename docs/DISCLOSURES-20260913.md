...
2026-09-13 사용자 요청으로 지분공시 메뉴·페이지·REST API·수집 기능을 제거했다.
인증키 미등록 상태에서 종료했으며 실제 공시 수집은 시작하지 않았다.
페이지는 워드프레스 휴지통으로 이동하고, 구현 파일은 Git 이력에 보존한다.
아래 내용은 종료 전 설계와 검증 기록이며 현재 운영 안내가 아니다.
