...
APPS = {
  'coin': dict(
    slug='coin', name='해외 코인 선물 챌린지', tint='coin', glow='rgba(240,185,11,0.11)',
    ...
    feats_lead='실제 거래소에서 처음 레버리지를 걸면 대부분 청산으로 배웁니다. 여기서는 같은 규칙으로 같은 값을 보되, 계좌만 가상입니다.',
  ...
  'krx': dict(
    slug='krx', name='국내주식 롱숏 챌린지', tint='krx', glow='rgba(49,130,246,0.11)',
    ...
    badge='출시됨 · 토스 미니앱',
    h1='국장에서<br>숏을 쳐봅니다',
    desc='개인은 공매도가 사실상 막혀 있습니다. 여기서는 코스피·코스닥 전 종목을 내리는 쪽에도 걸어볼 수 있습니다. 가상 자금 1,000만원으로 시작합니다.',
    ...
    shots=[('krx-1', '종목을 검색해 고르는 화면'), ('krx-2', '레버리지 5배로 삼성전자에 숏을 거는 주문 화면'),
           ('krx-3', '보유 포지션과 계좌 현황')],
    feats_h2='코인은 숏이 되는데<br>국장은 왜 안 될까요',
    feats_lead='궁금했던 것을 해보는 앱입니다. 실제 시장에서 못 하는 방향에 걸어 보고, 배수가 왜 무서운지 숫자로 봅니다.',
    feats=[('전 종목', '코스피 · 코스닥', '이름으로 검색해서 고릅니다. 상장된 종목은 다 있습니다.'),
           ('숏', '내리는 쪽에도', '개인은 막혀 있는 공매도를 여기서는 버튼 하나로. 오를지 내릴지 둘 다 걸어봅니다.'),
           ('100배', '레버리지', '0.5%만 반대로 가면 청산됩니다. 그 숫자를 청산가로 미리 보여줍니다.'),
  ...
  'saju': dict(
    slug='saju', name='사주 여덟 글자', tint='saju', glow='rgba(247,108,130,0.10)',
    ...
    hint='날씨만 기상청에서 받아오고 위치 정보는 쓰지 않습니다. 검색창에 ‘사주 여덟 글자’',
    ...
    feats=[('5초', '생년월일만', '시간을 모르면 몰라도 됩니다. 여덟 글자가 바로 세워집니다.'),
           ...
           ('0건', '서버로 나가는 개인정보', '생년월일·명식·궁합 상대 모두 폰 안에서만. 밖으로 나가는 건 날씨 요청 하나입니다.')],
    inout=True,
    steps=[('토스 앱을 엽니다', '따로 설치할 게 없습니다. 토스가 이미 있으면 끝입니다.'),
  ...
  'ai': dict(
    slug='ai', name='내 AI 활용 점수는?', tint='ai', glow='rgba(78,156,245,0.11)',
    ...
    feats=[('18', '문항 · 4분', '「복잡한 일을 시킬 때 어떻게 적나요」 같은 질문입니다. 정답은 없고, 해본 것만 고릅니다.'),
           ...
           ('3', '오늘 할 일', '처방 카드 세 장. 첫 장은 그냥 드리고, 나머지 둘은 광고 한 편을 보면 열립니다.'),
    ...
    notes=['<strong>점수는 자기 보고입니다.</strong> 해봤다고 고른 만큼 나옵니다. 후하게 고르면 후하게 나옵니다.',
           ...
           '처방 두 장을 여는 광고는 보상형이라, 안 보면 첫 장만 보이고 그 외에 막히는 건 없습니다.',
           '단계 이름은 공개 연구(arXiv 2608.07779)의 틀을 참고했고, 이 앱이 정한 것입니다.'],
    others=['coin', 'krx', 'saju', 'sudoku'],
  ),
  'sudoku': dict(
    slug='sudoku', name='오늘도 무료 스도쿠', tint='sudoku', glow='rgba(79,158,119,0.10)',
    ...
    shots=[],
    feats_h2='엔진을 직접 만들었습니다<br>그래서 판을 보증합니다',
    feats_lead='남의 퍼즐을 받아다 쓰면 답이 둘인 판이 섞입니다. 문제를 만들고 푸는 엔진을 직접 짜서, 답이 하나인 판만 내보냅니다.',
    feats=[('1개', '답은 하나뿐', '만든 판을 다시 풀어서 답이 하나임을 확인한 것만 내보냅니다. 찍어야 하는 자리가 없습니다.'),
           ('56', '통과한 테스트', '문제 생성·풀이·난이도 채점·힌트·시드 난수 — 엔진의 테스트 56개가 전부 통과합니다.'),
           ('오늘', '오늘의 퍼즐', '날짜가 씨앗입니다. 같은 날에는 누구나 같은 판을 받습니다.'),
           ('0KB', '서버 통신', '판은 기기 안에서 만듭니다. 지하철에서 연결이 끊겨도 하던 판이 그대로 이어집니다.'),
           ('꽉', '격자가 화면 전체', '상단 바를 없애고 격자를 화면 끝까지 채웠습니다. 숫자 하나하나가 큽니다.'),
           ('없음', '광고로 막는 자리', '힌트도 이어하기도 광고 없이 열립니다. 광고는 보고 싶은 사람만 봅니다.')],
    ...
    notes=['<strong>웹 버전은 앱과 판이 같습니다.</strong> 다만 랭킹과 진동은 브라우저에서 동작하지 않습니다.',
