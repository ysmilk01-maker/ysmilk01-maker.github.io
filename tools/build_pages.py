...
APPS = {
  'coin-info': dict(
    slug='coin-info', name='매수 전 코인 정보', tint='coin-info', glow='var(--coin-info-glow)',
    title='매수 전 코인 정보 · 나인투식스랩',
    meta='코인별 시세와 김치프리미엄, 롱숏 비율, 펀딩비와 미결제약정을 한 화면에서 확인합니다.',
    og='사기 전에 숫자의 뜻부터. 코인별 시세와 수급 정보를 한눈에 확인합니다.',
    badge='서비스 중 · 토스 미니앱',
    h1='사기 전에,<br>숫자의 뜻부터',
    desc='코인 이름이나 티커로 검색하고 시세와 차트를 확인합니다. 김치프리미엄, 롱숏 비율, 펀딩비와 미결제약정처럼 선물 거래 전에 알아야 할 숫자를 한 화면에 모았습니다.',
    hero_img='shots/coin-info-3.webp', hero_alt='매수 전 코인 정보의 코인별 심리 지수와 국내 대비 화면.', hero_width=636, hero_height=1048,
    how=['약 600종 시세', '김치프리미엄', '롱숏 비율', '펀딩비', '코인별 심리'],
    open_href='intoss://coin-before-buy', open_label='토스에서 열기',
    hint='토스 앱에서 열립니다. 검색창에 ‘매수 전 코인 정보’',
    shots=[('coin-info-1', '코인 이름과 티커로 검색하고 거래대금순으로 살펴보는 시세 목록.'),
           ('coin-info-2', '비트코인 차트와 전체 계정·상위 트레이더의 롱숏 비율.'),
           ('coin-info-3', '가격 흐름과 펀딩비, 롱숏 쏠림 등을 함께 본 코인별 심리 지수.')],
    feats_h2='같은 숫자도<br>뜻을 알고 봅니다',
    feats_lead='가격만 보지 않고 국내외 가격 차이와 선물 수급을 함께 살펴봅니다. 물음표를 누르면 각 지표가 무엇을 뜻하는지 설명합니다.',
    feats=[('약 600종', '시세 목록', '바이낸스 상장 코인을 이름과 티커로 검색합니다. 거래대금순, 변동률순, 김프순으로 바꿔 봅니다.'),
           ('5개', '차트 구간', '15분부터 주봉까지 캔들 차트를 확인합니다.'),
           ('2줄', '롱숏 비율', '전체 계정과 상위 트레이더가 어느 쪽에 더 많이 서 있는지 나란히 봅니다.'),
           ('국내 대비', '김치프리미엄', '업비트 원화 가격과 해외 가격의 차이, 52주 최고가와 최저가를 함께 표시합니다.'),
           ('선물 수급', '펀딩비 · 미결제약정', '다음 펀딩비 정산 시각, 마크가, 미결제약정과 시장가 매수·매도 비율을 확인합니다.'),
           ('0~100', '이 코인의 심리', '가격 흐름·펀딩비·롱숏 쏠림·변동성·거래량을 코인별로 따로 계산한 참고 지표입니다.')],
    status=[('토스', '서비스 중', '토스 앱에서 매수 전 코인 정보를 검색하거나 아래 버튼을 누르세요.'),
            ('시세', '공개 자료 조회', '바이낸스와 업비트 공개 자료를 받아 표시합니다. 거래소 상황에 따라 갱신이 늦을 수 있습니다.')],
    steps=[('토스 앱을 엽니다', '따로 설치할 게 없습니다. 토스가 이미 있으면 끝입니다.'),
           ('검색창에 이름을 칩니다', '‘매수 전 코인 정보’ 또는 ‘코인 정보’.'),
           ('관심 코인을 확인합니다', '시세와 차트, 국내 대비와 선물 수급을 차례로 살펴보세요.')],
    notes=['<strong>투자 조언이나 매매 신호를 제공하지 않습니다.</strong> 표시된 지표는 사용자가 정보를 해석하도록 돕는 참고 자료입니다.',
    ...
    others=['shooter', 'sudoku', 'coin', 'krx', 'ai', 'beatwave'],
  ),
  'coin': dict(
    ...
    others=['coin-info', 'beatwave', 'shooter', 'krx', 'ai', 'sudoku'],
  ),
  'krx': dict(
    ...
    others=['coin-info', 'beatwave', 'shooter', 'coin', 'ai', 'sudoku'],
  ),
  'saju': dict(
    ...
    others=['coin-info', 'beatwave', 'shooter', 'coin', 'krx', 'ai', 'sudoku'],
  ),
  'ai': dict(
    ...
    others=['coin-info', 'beatwave', 'shooter', 'coin', 'krx', 'sudoku'],
  ),
  'sudoku': dict(
    ...
    others=['coin-info', 'beatwave', 'shooter', 'coin', 'krx', 'ai'],
  ),
  'beatwave': dict(
    slug='beatwave',
    name='비트웨이브',
    ...
    others=['coin-info', 'shooter', 'sudoku', 'coin', 'krx', 'ai'],
  ),
  'shooter': dict(
    slug='shooter',
    name='오늘도 출격',
    ...
    others=['coin-info', 'beatwave', 'sudoku', 'coin', 'krx', 'ai'],
  ),
}

BLURB = {
  'coin-info': ('매수 전 코인 정보', '사기 전에 숫자의 뜻부터'),
